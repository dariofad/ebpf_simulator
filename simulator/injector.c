...
const double zeros[MAX_NOF_WISIGNALS] = {0, 0, 0, 0, 0, 0, 0, 0,};
...
int main(int argc, char **argv) {
	...
	rb = user_ring_buffer__new(fd, NULL);
        ...
        uint record_real_size =
            sizeof(uint) + sizeof(uint) + nof_signals * sizeof(double);
        uint record_size =
            sizeof(uint) + sizeof(uint) + MAX_NOF_WISIGNALS* sizeof(double);        
	struct model_record rec;
		...
		if (!p) {
			...
			continue;
                ...
                rec_p->time = rec.time;
		rec_p->filler = 0;                
                memcpy(rec_p->values, &(rec.values), 8 * MAX_NOF_WISIGNALS); // hardcoded
